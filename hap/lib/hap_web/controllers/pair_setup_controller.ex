...
defmodule HapWeb.PairSetupController do
  ...
  def pair_setup(%Conn{assigns: %{tlvs: [@setup_method, @m1]}} = conn, _params) do
    ...
    Logger.info(
      "sending m2 key_length: #{inspect(byte_size(public_key))} salt length: #{
        inspect(byte_size(salt))
      }"
    )
  ...
  def pair_setup(
        %Conn{assigns: %{tlvs: [@m3, client_public_key, client_srp_proof]}} = conn,
        _params
      ) do
    Logger.info("m3 received")

    response_data =
      case HAP.Pairing.pairing_m3(conn.remote_ip, client_public_key.value, client_srp_proof.value) do
  ...
  def pair_setup(
        %Conn{assigns: %{tlvs: [%TLV{type: :encrypted_data, value: encrypted_data}, @m5]}} = conn,
        _params
      ) do
    Logger.info("m5 received")

    response_data =
      case HAP.Pairing.pairing_m5(conn.remote_ip, encrypted_data) do
