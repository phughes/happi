...
defmodule TLVTest do
  ...
  test "single tlv to binary" do
    assert TLV.to_binary(%TLV{type: TLVType.state, value: 0}) == <<6, 1, 0>>
  end

  test "two tlv to binary" do
    assert TLV.to_binary([%TLV{type: TLVType.state, value: 0}, %TLV{type: TLVType.retry_delay, value: 1025}]) == <<6, 1, 0, 8, 2, 4, 1>>
  end
